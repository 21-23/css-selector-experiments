...
countRange n m p = count n p <> upto (m - n) p
...
data Combinator = ImmediateSibling | Child | Sibling | Descendant deriving Show
...
selector = do
  ...
  pure $ case more of
    ...
    pairs -> combined first
                where
                  {- magic lambda trick! -}
                  combined = foldr (\(c, sprev) sf snext -> Combined c snext (sf sprev)) id pairs
