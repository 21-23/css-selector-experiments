...
parseAndMatch :: Text -> Text -> Either (ParseErrorBundle Text Void) [Element]
parseAndMatch selectorText elementText = do
  sel <- parse Selector.selector "" selectorText
  el <- parse Element.element "" elementText
  pure $ matchTree sel (toZipper el)
